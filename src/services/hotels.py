...
class HotelsService:
    ...
    async def get_all(self, pagination):
        ...
        hotels = await self.db.hotels.get_all(limit=pagination.per_page, offset=pagination.offset)
        await self.redis.set(cache_key, json.dumps([h.model_dump() for h in hotels]), ex=300)
        return hotels
